...
def dashboard():
    ...
    # Use the new method that includes avatar URLs
    chats = db_manager.get_user_chats_with_avatars(user_profile_id)
...
def chat(chat_id):
    ...
    # Get the other user's avatar for individual chats
    if not chat_data.get('is_group', False) and len(chat_data['participants']) == 2:
        other_participant_id = None
        for participant_id in chat_data['participants']:
            if str(participant_id) != session['user_profile_id']:
                other_participant_id = participant_id
                break
        
        if other_participant_id:
            other_user = db_manager.get_user_profile(other_participant_id)
            if other_user and other_user.get('avatar_url'):
                chat_data['other_user_avatar'] = other_user['avatar_url']
...
# In the main block or initialization
if __name__ == '__main__':
    # Clean up duplicate chats on startup if the method exists
    if hasattr(db_manager, 'cleanup_duplicate_chats'):
        db_manager.cleanup_duplicate_chats()
    else:
        print("⚠️ cleanup_duplicate_chats method not available, skipping chat cleanup")
    
    print("Starting HangSpace server...")
